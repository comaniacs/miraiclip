...
- **Creative Features** — Keyframe animations (cubic-bézier easings) on transform/opacity/volume, shader effects (color adjust, blur, chroma key), transitions (cross dissolve, dips, wipe, slide) with equal-power audio crossfades, and reels-style karaoke captions with SRT/VTT and ASR word-timestamp import. [See them run live.](https://comaniacs.github.io/miraiclip/examples/) Effects and transitions are [extensible](https://comaniacs.github.io/miraiclip/docs/rendering/extensibility/): register your own kinds through the same contract the built-ins use.
