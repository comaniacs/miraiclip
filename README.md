...
- **Parameterized Videos** — [`@miraiclip/templates`](https://comaniacs.github.io/miraiclip/docs/templates/): declare fields on a project, feed it data rows, get personalized exports — placeholders in text/captions/HTML clips, asset slots, pure deterministic hydration with machine-readable failures, batch rendering through warm headless-Chrome sessions, and a `miraiclip-templates` CLI for JSON/NDJSON/CSV. Field schemas double as LLM tools.
