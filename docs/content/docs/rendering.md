...
Caption clips (`clip/add` with `kind: "caption"` — or the `captionClipsFromSubtitles` / `captionClipsFromAsrWords` importers, which turn an SRT/VTT file or ASR word timestamps into ordinary commands) render as a word-wrapped, centered block positioned by the clip's transform. Word-level timing drives the emphasis per the style `preset`: `plain` (no emphasis), `highlight` (the current word takes `highlightColor`), `karaoke` (words stay lit once passed — the reels look), and `pop` (the current word lights and enlarges in place). `fontSizeFrac` sizes text as a fraction of composition height, so a caption looks identical in a scaled preview and a full-resolution export; an optional `backgroundColor` draws a rounded box behind the block. Font assets (`asset/add` with `kind: "font"`, `src`, and the `family` clips reference) are loaded as real FontFaces by the player **and** by `exportProject` before the first frame renders — a server export never falls back to default glyphs.
