...
- **Parameterized videos** ✅ — `@miraiclip/templates`: a template project + declared fields + a data payload → a hydrated document → export. Placeholders bind into text, captions, and html-clip params; asset slots swap media; hydration is pure with machine-readable failures; field schemas double as LLM tools. Batch rendering runs rows through warm export sessions (`createExportSession` in server-export) with a `miraiclip-templates` CLI for JSON/NDJSON/CSV. Shipping as `@miraiclip/templates@0.1.0`. See [Templates](templates).
